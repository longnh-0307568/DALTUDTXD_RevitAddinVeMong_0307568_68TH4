...
namespace AddinVeMong.ViewModels
{
    public class EccentricRebarViewModel : INotifyPropertyChanged
    {
        ...
        #region 5. Thuộc tính Thép cổ cột & Đai
        ...
        public EccentricRebarViewModel(ExternalCommandData commandData)
        {
        ...
        private void ExecuteDrawRebar(object parameter)
        {
            try
            {
                ...
                {
                    TaskDialog.Show("Thông báo", "Vui lòng chọn một cấu kiện Móng đơn lệch tâm trước khi bấm Tạo Thép!");
                    return;
                }
                ...
                {
                    TaskDialog.Show("Lỗi dữ liệu", "Đối tượng được chọn không phải là Móng đơn. Vui lòng chọn lại!");
                    return;
                }
                ...
                if (boundingBox != null)
                {
                    ...
                    {
                        columnElement = connectedColumns.First();
                    }
                }
                ...
                {
                    TaskDialog.Show("Lỗi Family", "Không tìm thấy thông số thuộc tính kích thước trong Family móng.");
                    return;
                }
                ...
                // Tọa độ tâm cột dựa trên độ lệch tâm
                XYZ eccentricCenter = footingCenter + uX * eccXFoot + uY * eccYFoot;

                View3D activeView3D = _doc.ActiveView as View3D;

                using (Transaction trans = new Transaction(_doc, "Tạo Toàn Bộ Thép Móng Lệch Tâm và Cổ Cột"))
                {
                    trans.Start();
                    ...
                    double shortDiaFoot = UnitUtils.ConvertToInternalUnits(this.ShortDiameter, UnitTypeId.Millimeters);

                    double zStarterBottom = zBottom + longDiaFoot + shortDiaFoot;
                    
                    // SỬ DỤNG eccentricCenter TẠI ĐÂY
                    XYZ starterBaseCenter = eccentricCenter + uZ * zStarterBottom;
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        ...
                        if (starterBarType != null)
                        {
                            ...
                            if (starterRebar != null)
                            {
                                SetRebarSolid3D(starterRebar, activeView3D);
                            }
                        }
                    }

                    // 3. Thép đai cổ cột - ĐẶT TẠI TÂM LỆCH (eccentricCenter)
